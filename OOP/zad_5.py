...
class Basket:
    def __init__(self):
        self._items = dict()  # pusty slownik z itemami w koszyki
    ...
    def add_product(self, product: Product, quantity: int):
        ...
        if product in self._items:
            self._items[product] += quantity  # kluczem jest produkt, wartoscia ilosc produktow
        else:
    ...
    def count_total_price(self) -> float:
        ...
        total_price = 0.0
        for product, quantity in self._items.items():  # dla kazdego klucza i wartosci w slowniku imets
